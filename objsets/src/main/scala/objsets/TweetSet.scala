...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    ...
    val rest = remove(tw)
    new Cons(head=tw, tail=rest.descendingByRetweet)
  }
...
object GoogleVsApple {
  ...
  def filterTweetsContainingTerms(stuff: TweetSet, terms: List[String]): TweetSet = {
    stuff.filter(t => terms.exists( s => t.text.contains(s)))
  }

  lazy val googleTweets: TweetSet = filterTweetsContainingTerms(TweetReader.allTweets, google)
  lazy val appleTweets: TweetSet = filterTweetsContainingTerms(TweetReader.allTweets, apple)
